/*
 ### jQuery Multiple File Upload Plugin v1.26 - 2008-02-12 ###
   ...
   MultiFile: function( o /* Object */ ){
    ...
    if(!$.MultiFile.intercepted) (function(ai){
     ...
     for(var i=0;i<ai.length;i++){
      var func = ai[i]+'';
      if(func) (function(func){ // MAKE SURE THAT func IS ISOLATED for the interception
       //if(console) console.log('TEST: Install interception for '+func+'');
       $.MultiFile.intercepted[func] = $.fn[func] || function(){};
       $.fn[func] = function(){
        //if(console) console.log(['MultiFile','intercept:',ai,i,arguments]);
        //if(console) console.log(['TEST: Intercepeted '+func+'']);
        $('input:file.MultiFile').each(function(){
         var $this = $(this);
         if($this.val()=='') $this.addClass('mfD').each(function(){ this.disabled = true });
        });
        //if(console) console.log(['TEST: Executing original '+func+'']);
        $.MultiFile.intercepted[func].apply( this, arguments );
        //if(console) console.log(['TEST: Clean up after intercepting '+func+'']);
        $('input:file.mfD').removeClass('mfD').each(function(){ this.disabled = false });
        return this;
       }; // interception
      })(func); // MAKE SURE THAT func IS ISOLATED for the interception
     };// for each ai
    })([ 'ajaxSubmit', 'validate' /* array of methods to intercept */ ]);
				//if(console) console.log(['TEST: DONE:',$.MultiFile.intercepted]);
    //###
       ...
       MASTER.addToList = function( slave ){
        ...
        b.click(function(){
          ...
          // Show most current element again (move into view) and clear selection
										$(MASTER.curEle).css({ position:'', left: '' }).reset().val('').attr('value', '')[0].value = '';
