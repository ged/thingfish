...
module ThingFish::Benchmark
	...
	class Task < Rake::Task
		...
		def self::define_task( *args, &block )
			...
			directory task.output_dir.to_s
		...
		def save_dataset
			savefile = @output_dir + "%s.data" % [ benchmarkname() ]
...
def benchmark( *args, &block )
	task = ThingFish::Benchmark::Task.define_task( *args, &block )
end
